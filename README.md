...
会话服务通过 `RAG_BASE_URL` 定位 RAG 服务，默认 `http://127.0.0.1:8010`。RAG 服务通过 `AUTHZ_BASE_URL` 定位权限服务，默认 `http://127.0.0.1:8012`；配置 `AUTHZ_BASE_URL` 后 RAG 才会启用权限强制。三个服务共享同一个 `DATABASE_URL`（同一 PostgreSQL，各自只触碰自己的表）。
...
POST /api/v1/authz/visible-scope            # 数据级可见范围 {principal, kb_id, project_id} -> {allowed, project_id, scope_context}
...
`retrieve` 接收 `question`、`kb_id`、`project_id`、`top_k`，返回 `{chunks:[{id,filename,chunk_index,score,content,summary}], retrieved}`。`ask` 额外接收 `history`（内联 `[{role, content}]`）和 `stream`；非流式返回 `{answer, answer_mode, citations, retrieved}`。检索/取片的行级可见性由 RLS 强制：RAG 只透传调用方在 `x-scope-context` 头里给出的不透明可见范围（默认 `*` 全项目），该范围由 authz 的 `visible-scope` 计算。`chat/completions` 接收 `session_id`、`session_token`、`question` 及 scope，以 SSE 返回 `sources`、`delta`、`done` 或 `error` JSON 事件；会话消息保存在 PostgreSQL `chat_messages`，生成时只读取最近 12 条。
...
2. **数据级**：调用方调 `authz` 的 `visible-scope` 得到不透明 `scope_context`（`*` 表示全项目，否则为逗号分隔的可见部门 id，含部门树继承与公共部门），把它放进 `x-scope-context` 头传给 RAG。RAG 只把这个字符串 `set_config` 进数据库事务，由 authz 拥有的 **Postgres RLS 策略**在行级过滤 `knowledge_evidence`——RAG 的检索/取片 SQL 里没有任何权限谓词，也不含 `access_scope` 列参与访问判定（`access_scope` 只是 RAG 不认识语义的不透明访问键）。
